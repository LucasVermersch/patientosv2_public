...
class LitRepository extends ServiceEntityRepository
{
    ...
    public function search($service, $chambre){
        return $this->createQueryBuilder('lit')
            ->join('lit.Chambre','c')
            ->join('c.Service','s')
            ->andWhere('s.NomService LIKE :service')
            ->andWhere('c.id LIKE :id')
            ->setParameter('id','%' . $chambre . '%')
